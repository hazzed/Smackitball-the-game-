...
public class TestGUI : MonoBehaviour
{
    ...
    private void ShowSettings(float w)
    {
        ...
        bool filtering = GUILayout.Toggle(SixenseCore.Device.FilterEnabled, "Moving Average Filter");
        SixenseCore.Device.FilterEnabled = filtering;

        if (filtering)
        {
            GUILayout.BeginHorizontal();
            {
                var settings = SixenseCore.Device.FilterSettings;
                ...
                if (c) GUI.changed = false;
                {
                    ...
                    string nvs = GUILayout.TextArea(settings.NearValue.ToString("0.0##"));
                    GUILayout.Label("-");
                    string fvs = GUILayout.TextArea(settings.FarValue.ToString("0.0##"));

                    if (GUI.changed &&
                        float.TryParse(nrs, out settings.NearRange) &&
                        float.TryParse(nvs, out settings.NearValue) &&
                        float.TryParse(frs, out settings.FarRange) &&
                        float.TryParse(fvs, out settings.FarValue))
                    {
                        SixenseCore.Device.FilterSettings = settings;
                    }
                }
                GUI.changed |= c;
            }
    ...
    private void ShowPnO(float w)
    {
        ...
        for (int i = 0; i < SixenseCore.Device.MaxNumberTrackers; i++)
        {
            ...
            GUILayout.BeginHorizontal();
            {
                GUILayout.Label(con.DeviceIndex + " position (mm):", GUILayout.Width(2 * w / 5 - 1));
                var xyz = con.Position * 1000;
                GUILayout.Label("x " + Mathf.RoundToInt(xyz.x).ToString() + ",", GUILayout.Width(w / 5 - 1));
                GUILayout.Label("y " + Mathf.RoundToInt(xyz.y).ToString() + ",", GUILayout.Width(w / 5 - 1));
                GUILayout.Label("z " + Mathf.RoundToInt(-xyz.z).ToString(), GUILayout.Width(w / 5 - 1));
            }
            GUILayout.EndHorizontal();
            GUILayout.BeginHorizontal();
            {
                GUILayout.Label("   rotation (deg):", GUILayout.Width(2 * w / 5 - 1));
                var pyr = getPitchYawRoll(con.Rotation);
                GUILayout.Label("ex " + Mathf.RoundToInt(pyr.x).ToString() + ",", GUILayout.Width(w / 5 - 1));
                GUILayout.Label("ey " + Mathf.RoundToInt(pyr.y).ToString() + ",", GUILayout.Width(w / 5 - 1));
                GUILayout.Label("ez " + Mathf.RoundToInt(pyr.z).ToString(), GUILayout.Width(w / 5 - 1));
            }
            GUILayout.EndHorizontal();
            GUILayout.BeginHorizontal();
            {
                GUILayout.Label("   gravity (m/s/s):", GUILayout.Width(2 * w / 5 - 1));
                var xyz = con.LocalGravity;
                GUILayout.Label("x " + (xyz.x).ToString("0.00") + ",", GUILayout.Width(w / 5 - 1));
                GUILayout.Label("y " + (xyz.y).ToString("0.00") + ",", GUILayout.Width(w / 5 - 1));
                GUILayout.Label("z " + (-xyz.z).ToString("0.00"), GUILayout.Width(w / 5 - 1));
            }
            GUILayout.EndHorizontal();
            GUILayout.Space(5);
